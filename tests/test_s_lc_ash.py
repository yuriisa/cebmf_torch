...
from cebmf_torch.cebnm import fit_new_trait, s_lc_ash_posterior_means
...
def _simulate_panel(
    ...
    *,
    ...
    if c_per_trait is None:
        c_per_trait = [1.0] * n_traits
    rows = []
    ...
    return {
        ...
        "n_traits": n_traits,
        "c_per_trait": np.array(c_per_trait),
    }
...
class TestMarginalDensity:
    def test_matches_scipy_handcrafted(self):
        ...
        trait_id = torch.tensor([r[0] for r in records], dtype=torch.long)
        ...
        log_m = s_lc_ash_log_marginal(beta, se, trait_id, log_c, logit_p, eta, sigma_t)
    ...
    def test_finite_for_extreme_se(self):
        ...
        trait_id = torch.zeros_like(beta, dtype=torch.long)
        log_m = s_lc_ash_log_marginal(beta, se, trait_id, log_c, logit_p, eta, sigma_t)
    ...
    def test_rejects_zero_sigma(self):
        ...
        trait_id = torch.zeros(1, dtype=torch.long)
        with pytest.raises(ValueError, match="strictly positive"):
            s_lc_ash_log_marginal(beta, se, trait_id, log_c, logit_p, eta, sigma_t)
...
class TestPosteriors:
    def test_matches_scipy(self):
        ...
        eta = torch.log(torch.tensor(w, dtype=torch.float64))
        sigma_t = torch.tensor(sigma, dtype=torch.float64)
        out = s_lc_ash_compute_posteriors(beta, se, trait_id, log_c, logit_p, eta, sigma_t)
        for i, (tid, b, s) in enumerate(records):
    ...
    def test_spike_at_column_zero(self):
        ...
        trait_id = torch.zeros(3, dtype=torch.long)
        out = s_lc_ash_compute_posteriors(beta, se, trait_id, log_c, logit_p, eta, sigma_t)
...
class TestWarmStart:
    def test_strips_spike_entry(self):
        ...
        warm = warm_start_from_pooled_ash(betahat, se, ash_init=False)
...
class TestPanelFit:
    def test_returns_cash_posterior_mean_norm_with_expected_fields(self):
        sim = _simulate_panel(n_per_trait=200, n_traits=4, seed=1)
        res = s_lc_ash_posterior_means(
            sim["betahat"], sim["sebetahat"], sim["X_cat"], sim["n_traits"],
        ...
        assert res._arch_meta["T"] == sim["n_traits"]
        psi = res.priors_fitted[0]
        for k in ("mu_c", "tau2_c", "p", "solver"):
            assert k in psi
        tp = res.trait_params
        assert tp["c"].shape == (sim["n_traits"],)
        assert tp["p"].shape == (1,)
    ...
    def test_recovers_known_c_correlated(self):
        ...
        true_c = np.exp(rng.normal(0.0, 0.7, size=n_traits))
        sim = _simulate_panel(
            n_per_trait=600, n_traits=n_traits,
            c_per_trait=true_c.tolist(), p_global=0.7, seed=7,
        )
        res = s_lc_ash_posterior_means(
            sim["betahat"], sim["sebetahat"], sim["X_cat"], n_traits,
            n_epochs=400, verbose=False, seed=7,
        )
        c_hat = res.trait_params["c"].numpy()
    ...
    def test_seed_reproducibility(self):
        sim = _simulate_panel(n_per_trait=200, n_traits=4, seed=3)
        ...
        torch.testing.assert_close(res_a.trait_params["c"], res_b.trait_params["c"],
                                   rtol=1e-12, atol=1e-12)

    def test_homogeneous_panel_collapses_tau2_c_to_floor(self):
        sim = _simulate_panel(
            n_per_trait=800, n_traits=10,
            c_per_trait=[1.0] * 10, p_global=0.7, seed=11,
        )
        res = s_lc_ash_posterior_means(
    ...
    def test_heterogeneous_panel_keeps_tau2_c_above_floor(self):
        sim = _simulate_panel(
            n_per_trait=800, n_traits=10,
            c_per_trait=[0.3, 0.5, 0.8, 1.0, 1.2, 1.5, 2.0, 2.5, 3.0, 4.0],
    ...
    def test_predict_pi_raises(self):
        sim = _simulate_panel(n_per_trait=80, n_traits=2, seed=8)
        res = s_lc_ash_posterior_means(
            sim["betahat"], sim["sebetahat"], sim["X_cat"], 2, n_epochs=20, verbose=False,
        )
        with pytest.raises(NotImplementedError, match="fit_new_trait"):
            res.predict_pi(X_cat=torch.tensor([0, 1], dtype=torch.long))

    def test_state_dict_kernel_round_trip(self):
        sim = _simulate_panel(n_per_trait=100, n_traits=3, seed=5)
        res = s_lc_ash_posterior_means(
    ...
    def test_rejects_invalid_inputs(self):
        sim = _simulate_panel(n_per_trait=20, n_traits=2, seed=8)
        with pytest.raises(ValueError, match="strictly positive"):
    ...
    def test_track_loglik_history_does_not_change_posteriors(self):
        sim = _simulate_panel(n_per_trait=150, n_traits=3, seed=4)
...
class TestFitNewTrait:
    def _fit_panel(self, **kw):
        sim = _simulate_panel(
            n_per_trait=600, n_traits=10,
            c_per_trait=[0.3, 0.5, 0.8, 1.2, 1.5, 2.0, 2.5, 3.0, 0.7, 1.8],
    ...
    def test_returns_cash_posterior_mean_norm(self):
        ...
        mask = sim["X_cat"] == 0
        new = fit_new_trait(sim["betahat"][mask], sim["sebetahat"][mask], panel,
        ...
        assert new._arch_meta["single_trait"] is True
        assert new.trait_params["c"].shape == (1,)

    def test_does_not_mutate_panel(self):
        sim, panel = self._fit_panel()
        c_before = panel.trait_params["c"].clone()
        psi_before = dict(panel.priors_fitted[0])
        mask = sim["X_cat"] == 3
        _ = fit_new_trait(sim["betahat"][mask], sim["sebetahat"][mask], panel,
                            n_epochs=100, verbose=False, seed=40)
        torch.testing.assert_close(panel.trait_params["c"], c_before, rtol=0, atol=0)
    ...
    def test_round_trip_recovers_panel(self):
        sim, panel = self._fit_panel()
        c_panel = panel.trait_params["c"].numpy()
        c_cold = []
        for t in range(sim["n_traits"]):
            mask = sim["X_cat"] == t
            new = fit_new_trait(sim["betahat"][mask], sim["sebetahat"][mask], panel,
                                 n_epochs=300, verbose=False, seed=40)
            c_cold.append(float(new.trait_params["c"][0]))
    ...
    def test_tau_inflate_widens_prior(self):
        ...
        new1 = fit_new_trait(b, s, panel, tau_inflate=1.0, n_epochs=300, verbose=False, seed=40)
        new3 = fit_new_trait(b, s, panel, tau_inflate=3.0, n_epochs=300, verbose=False, seed=40)
        d1 = abs(math.log(float(new1.trait_params["c"][0])) - mu_c)
        d3 = abs(math.log(float(new3.trait_params["c"][0])) - mu_c)
        assert d3 >= d1 - 1e-3
    ...
    def test_rejects_non_s_lc_ash_panel(self):
        ...
        with pytest.raises(ValueError, match="s_lc_ash_posterior_means"):
            fit_new_trait(torch.zeros(3), torch.ones(3), bad)

    def test_single_observation_trait(self):
        sim, panel = self._fit_panel()
        b = sim["betahat"][:1]
        s = sim["sebetahat"][:1]
        new = fit_new_trait(b, s, panel, n_epochs=100, verbose=False, seed=42)
        assert math.isfinite(new.marginal_loglik)
        c_hat = float(new.trait_params["c"][0])
        mu_c = float(panel.priors_fitted[0]["mu_c"])
    ...
    def test_predict_pi_raises_for_single_trait(self):
        sim, panel = self._fit_panel()
        mask = sim["X_cat"] == 0
        new = fit_new_trait(sim["betahat"][mask], sim["sebetahat"][mask], panel,
...
class TestFloat32:
    def test_kernel_float32_close_to_float64(self):
        ...
        log_m_64 = s_lc_ash_log_marginal(beta64, se64, trait_id, log_c64, logit_p64, eta64, sigma_t64)
        log_m_32 = s_lc_ash_log_marginal(
            beta64.float(), se64.float(), trait_id,
